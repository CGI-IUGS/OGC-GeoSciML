...
Figure 23 shows an example of GeoSciML encoding of a landform lava cone geomorphologic unit (from <<ref2>>) located in Elba Island showing how the geomorphologic feature is related to the geologic unit and materials that it is made of.
...
The Geologic Time package, developed by Simon Cox (CSIRO) and Steve Richard (Arizona Geological Survey) <<ref1>>, contains elements used to describe the classification of geologic time: time periods, time boundaries, and the relationships between them as defined by the IUGS International Commission on Stratigraphy (ICS - http://www.stratigraphy.org/).
